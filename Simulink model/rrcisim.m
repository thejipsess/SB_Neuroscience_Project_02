...
% old: [x,Srate,bpsa] = audioread(join(['audio_in/', filename]));
[y, Srate] = audioread(join(['audio_in/', filename]));

% Resample audio to 12971 Hz
x = resample(y, 12971, Srate);
...
wavname = ['_ch',num2str(nch),'_BPF',num2str(bpflow),'_RECT',recttype,'_LPF',num2str(lpftype),'.flac'];
name = ['audio_out/',outdir,wavname];
audiowrite(name, youtsum, 12971)
% old: wavwrite(youtsum,Srate,bpsa,['audio_out/',outdir,wavname]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
